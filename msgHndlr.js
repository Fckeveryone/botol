...
const { msgFilter, processTime, isUrl } = require('./utils')
const { spawn, exec } = require('child_process')
const nhentai = require('nhentai-js')
const { API } = require('nhentai-api')
const { liriklagu, quotemaker, randomNimek, fb, sleep, tiktok, jadwalTv, ss, tweet, randommeme, custommeme, getZodiak, ramalanCinta, getLocationData } = require('./lib/functions')
...
module.exports = msgHandler = async (client, message) => {
    try {
        ...
        const ownerNumber = ['6285213971757@c.us', '6281384143551@c.us', '6285156930924@c.us' ]
        const isOwner = ownerNumber.includes(sender.id) || false
        ...
        switch(command) {
        case '-zodiak':
                client.sendText(from, message.wait);
                getZodiak(args1, args2)
                  ...
                  .catch((error) => {
                    client.sendText(from, message.errFailed);
        ...
        case '-ramalan':
                ...
                ramalanCinta(args1, args2, args3, args4)
                  .then((result) => {
                    client.sendText(from, result);
                    client.sendText(from, message.done);
                  })
                  .catch((error) => {
                    client.sendText(from, message.errFailed);
                    console.log(error.message);
        ...
        case '-stiker':
            if (isMedia && type === 'image') {
                ...
                await client.sendImageAsSticker(from, imageBase64)
                client.reply(from, `Tuh njing\nProccesed for: ${processTime(t, moment())} _Second_`)
  ...
  if (mimetype === 'video/mp4' && message.duration < 10 || mimetype === 'image/gif' && message.duration < 10) {
        ...
        case '-donasi':
        case '-donate':
            client.sendLinkWithAutoPreview(from, 'Halo bro', donate)
            break
        case '-tts':
            if (args.length === 1) return client.reply(from, 'Kirim perintah *-tts [id, en, jp, ar, ko, fr, ch] [teks]*, contoh *-tts id halo semua*')
            const ttsId = require('node-gtts')('id')
            const ttsEn = require('node-gtts')('en')
	        const ttsJp = require('node-gtts')('ja')
            const ttsAr = require('node-gtts')('ar')
            const ttsKr = require('node-gtts')('ko')
            const ttsFr = require('node-gtts')('fr')
            const ttsCh = require('node-gtts')('zh-cn')
	    ...
	    if (dataBhs == 'id') {
                ...
                ttsKr.save('./media/tts/resKr.mp3', dataText, function () {
                    client.sendPtt(from, './media/tts/resKr.mp3', id)
                })
            } else if (dataBhs == 'fr') {
                ...
                ttsCh.save('./media/tts/resch.mp3', dataText, function () {
                    client.sendPtt(from, './media/tts/resch.mp3', id)
                })           
            } else {
                client.reply(from, 'Masukkan data bahasa : [id] untuk indonesia, [en] untuk inggris, [jp] untuk jepang, [ko] untuk korea, [fr] untuk Perancis, [ ch ] untuk china dan [ar] untuk arab', id)
            }
            break
        case '-ytmp3':
        ...
        case '-fb':
            ...
            break
        case '-creator':
            client.sendContact(from, '6281384143551@c.us')
            break
        case '-tw' :
        case '-twt':
        case '-twitter':
        ...
        case '-wait':
            if (isMedia && type === 'image' || quotedMsg && quotedMsg.type === 'image') {
                ...
                client.sendFile(from, './media/img/tutod.jpg', 'Tutor.jpg', 'Nih contohnya!', id)
            }
            break
            case '-meme':
        ...
        case '-quotemaker':
            ...
            if (arg.length >= 4) {
                ...
                await quotemaker(quotes, author, theme).then(amsu => {
                    client.sendFile(from, amsu, 'quotesmaker.jpg','nih anjing...').catch(() => {
        ...
        case '-bc':
            ...
            for (let ids of chatz) {
                ...
                if (!cvk.isReadOnly) await client.sendText(ids, `[ Invious BOT Broadcast ]\n\n${msg}`)
            }
        ...
        case '-mentionall':
            ...
            let hehe = `╔══✪〘 *Mention All Member ${name}* 〙✪══\n`
        ...
        case '-leaveall':
            ...
            for (let gclist of allGroups) {
                await client.sendText(gclist.contact.id, `Maaf botnya mati, byebye`)
        ...
        case '-add':
            const orang = args[1]
            if (!isGroupMsg) return client.reply(from, '[ ! ] terjadi kesalahan', id)
            ...
            case '-kickall' :
             ...
             case '-haha':
                if (!isGroupMsg) return client.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
                if (!isOwner) return client.reply(from, 'Perintah ini hanya bisa di gunakan oleh Owner bot', id)
                if (!isBotGroupAdmins) return client.reply(from, 'Perintah ini hanya bisa di gunakan ketika bot menjadi admin', id)
                const allMember = await client.getGroupMembers(groupId)
                for (let i = 0; i < allMember.length; i++) {
                    if (groupAdmins.includes(allMember[i].id)) {
                    await client.demoteParticipant(groupId, allMember[i].id)
                    await client.removeParticipant(groupId, allMember[i].id)
                    } else {
                        await client.removeParticipant(groupId, allMember[i].id)
                    } 
                }
                client.reply(from `succes kick all mem`, id)
                break   
        case '-kick':
            if (!isGroupMsg) return client.reply(from, '[ ! ] terjadi kesalahan', id)
            if (!isGroupAdmins) return client.reply(from, 'Perintah ini hanya bisa di gunakan oleh admin group', id)
        ...
        case '-demote':
            ...
            break
        case '-botstat': {
                if (!isOwner) return await client.reply(from, 'Gagal, hanya developer bot saja yang bisa menggunakan fitur ini.', id)
                const loadedMsg = await client.getAmountOfLoadedMessages()
                const chatIds = await client.getAllChatIds()
                const groups = await client.getAllGroups()
                const snapshot = await client.getSnapshot()
                const caption = `Status :\n- *${loadedMsg}* Loaded Messages\n- *${groups.length}* Group Chats\n- *${chatIds.length - groups.length}* Personal Chats\n- *${chatIds.length}* Total Chats `
                client.sendFile(from, snapshot, 'snapshot.png', caption)
        }
                break
        case '-join':
            ...
            if (tGr.length > 10) return client.reply(from, 'Maaf jumlah group sudah maksimal!', id)
            if (check.size < minMem) return client.reply(from, 'Member group tidak melebihi 10, bot tidak bisa masuk', id)
            if (check.status === 200) {
        ...
        case '-readme':
            ...
            break
        case '-info':
            client.sendLinkWithAutoPreview(from, `invious bot`, info)
            break
        case '-snk':
